    ...
    return async (op, jsonArgs) => {
        ...
        switch (op) {
            ...
            case "1.0/runUdf": {
                ...
                if (udfType === "query") {
                    return JSON.stringify(convexToJson(await withAuth().queryFromPath(functionPath, udfArgs)));
                }
                if (udfType === "snapshotQuery") {
                    return JSON.stringify(convexToJson(await withAuth().snapshotQueryFromPath(functionPath, udfArgs)));
                }
                if (udfType === "mutation") {
                    return JSON.stringify(convexToJson(await withAuth().mutationFromPath(functionPath, udfArgs)));
                }
...
function withAuth(auth = authStorage.getStore() ?? new AuthFake()) {
    ...
    const runMutationWithHandler = async (handler, args, extraCtx = {}, functionPath, isNested) => {
    ...
    const runQueryWithHandler = async (handler, args, functionPath, isNested) => {
    ...
    const byTypeWithPath = {
        queryFromPath: async (functionPath, args) => {
            ...
            try {
                ...
                const result = await runQueryWithHandler(resolved.handler, args, resolved.functionPath, isNested);
        ...
        snapshotQueryFromPath: async (functionPath, args) => {
            const parentLock = nestedTxStorage.getStore() ?? null;
            if (parentLock) {
                await parentLock.acquire();
            }
        ...
        mutationFromPath: async (functionPath, args) => {
            const parentLock = nestedTxStorage.getStore() ?? null;
            const isNested = parentLock !== null;
            if (parentLock) {
                await parentLock.acquire();
            ...
            try {
                ...
                validateValidator(resolved.args, args ?? {});
                const result = await runMutationWithHandler(resolved.handler, args, {}, resolved.functionPath, isNested);
